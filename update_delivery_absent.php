<?php
    ...
    if(isset($_POST)){
        ...
        $queryInactivePatient = "update `patient`,`order` set `patient`.`actief` = 1 where `order`.`id` = ".$_POST["order"]." and `order`.`akng` = 2 and `order`.`patient` = `patient`.`id`;";
        $resultInactivePatient = mysqli_query($conn, $queryInactivePatient);
