<?php
  ...
  function test(){
    ...
    function get_patient(){
        ...
        //var_dump($result);
        while($record = mysqli_fetch_assoc($result)){
              echo "<tr><td>".$record['achternaam'].", ".$record['tussenvoegsel']."</td>
                        <td>".$record['vzknr']."</td>
                        <td>".$record['geboortedatum']."</td>
                        <td>".$record['tel']."</td>
                        <td>".$record['wnplts'].", ".$record['straat'].", ".$record['hsnr'].", ".$record['postcode']."</td>";
                        if($record['actief'] == 0){
                        echo "<td><form action='blocked_database.php' method='post'><select name='blocked'><option value='1'>Ja</option><option value='0' selected>Nee</option></select><button type='submit' class='btn login_btn' name='patient' value='".$record['id']."'>Opslaan</button></form></td></tr>";
                        }
                        else{
                            echo "<td><form action='blocked_database.php' method='post'><select name='blocked'><option value='1' selected>Ja</option><option value='0'>Nee</option></select><button type='submit' class='btn login_btn' name='patient' value='".$record['id']."'>Opslaan</button></form></td></tr>";
                        }
        }
    }
            ...
            case "patient":
                echo "<div class=\"row upperHomeRow\">
                        <div class=\"col-lg-12\">
                            ...
                            <div class=\"col-lg-12\">
                            <div class=\"col-lg-2 homeButton\" onclick=\"location.href='order_overview.php';\">Bestel overzicht</div>
                        </div>
                     </div>";
                break;
            ...
            case "huisarts":
                echo "<div class=\"row upperHomeRow\">
                        <div class=\"col-lg-12\">
                            ...
                            <div class=\"col-lg-1\"></div>
                            <div class=\"col-lg-2 homeButton\" onclick=\"location.href='patient_overview.php';\">Patiënt overzicht</div>
                        </div>
                     </div>";
                break;
        ...
        while($record = mysqli_fetch_assoc($result)){
            echo"
                <tr>
                    ...
                    <td class=\"col-lg-02\">".$record['beschrijving']."</td>
                    <td class=\"col-lg-02\">".$record['voorraad']."</td>";
                    if($record['verzekerd'] == 0){
                        echo "<td class=\"col-lg-02\">nee</td>";
                    }
                    else{
                        echo "<td class=\"col-lg-02\">ja</td>";
                    }
            echo "  <td><input name='medicijn[]' value='".$record['id']."' class='checkbox' type='checkbox'></td>
                </tr>";
...
*///////////////////////////////////////////////////////////
    ...
    function get_active_orders(){
        ...
        //$query = "select order.id, order.patient, order.bedrag, order.btlmet, order.akng, order.orderstatus from order INNER JOIN orderregels on orderregels.order = order.id where order.patient = ".$_SESSION["id"]." and order.orderstatus = 1 ;";
        
        $query = "Select id, orderstatus from `order` order by id;";
        ...
        while($record = mysqli_fetch_assoc($result)){
            echo "
            <tr>
                <td class=\"col-lg-2\">".$record["id"]."</td>
                <td class=\"col-lg-2\"></td>
                <td class=\"col-lg-2\"></td>
                <td class=\"col-lg-1 tableButtons\" onclick=\"location.href='active_order_info.php?bestelnr=".$record["id"]."';\">Info</td>
                <td class=\"col-lg-2 tableButtons\" onclick=\"location.href='active_order_edit.php?bestelnr=".$record["id"]."';\">Levering wijzigen</td>
            </tr>";
        }
    }
...
*///////////////////////////////////////////////////////////
    ...
    function get_active_deliveries(){
        ...
        $query = "SELECT distinct `order`.`akng`, `order`.id, `orderregels`.`datum` AS `datum`, `orderregels`.`tijd` AS `tijd`, `patient`.voornaam, `patient`.tussenvoegsel, `patient`.achternaam, `patient`.straat, `patient`.hsnr, `patient`.postcode, `patient`.wnplts, `patient`.tel FROM `order` LEFT JOIN `orderregels` ON `order`.`id` = `orderregels`.`order` LEFT JOIN `patient` ON `order`.`patient` = `patient`.`id` WHERE `order`.`orderstatus` = 1 and `order`.`orderstatus` = 1 and `orderregels`.datum = (select o.datum from `orderregels` as o where o.datum = '".date("Y-m-d")."' and o.orderActief = 1 and `order`.`akng` < 2  order by o.datum limit 1) order by `datum`;";
...
*///////////////////////////////////////////////////////////
    ...
    function get_apotheek_admin(){
        ...
        while($record = mysqli_fetch_assoc($result)){
            ...
            <tr>
                <td class=\"col-lg-02\">".$record["naam"]."</td>
                <td class=\"col-lg-04\">".$record["straat"]." ".$record["hsnr"].", ".$record["postcode"]." ".$record["woonplaats"]."</td>
